  ...
  try {
    ...
    res.status(500).json({
      ...
      error: process.env.NODE_ENV === 'development' ? error.message : undefined
        ...
        message: '⚠️ Mock login successful (database not connected)'
      });
    }
...
app.post('/api/posts', authenticateToken, async (req, res) => {
  try {
    ...
    if (!dbConnected) {
      // Mock post creation when database is not connected
      const mockPost = {
        _id: 'mock-post-' + Date.now(),
        author: {
          _id: req.userId,
          name: req.user?.email?.split('@')[0] || 'Test User',
          avatar: null
        },
        content,
        mediaUrl,
      ...
      return res.status(201).json({
        success: true,
        message: '⚠️ Mock post created successfully (database not connected)',
        data: { post: mockPost }
      });
    }

    // Real database post creation
    const post = new Post({
      author: req.userId,
      content,
      mediaUrl,
...
app.get('/api/posts', async (req, res) => {
  try {
    ...
    if (!dbConnected) {
      // Return mock posts data when database is not connected (log once)
      if (!global.mockDataLogged) {
        console.log('📝 Using mock posts data (database not connected)');
        global.mockDataLogged = true;
      }
      const mockPosts = [
        {
          _id: 'mock-post-1',
          ...
          content: 'Welcome to TravelSocial! 🌍 This is a demo post since the database is not connected.',
          mediaUrl: 'https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=500&h=300&fit=crop',
          mediaType: 'image',
        ...
        },
        {
          ...
          content: 'Amazing sunset at the beach! 🌅 Can\'t wait to share more travel experiences.',
      ...
      return res.status(200).json({
        ...
        message: '⚠️ Mock data - database not connected'
      });
    }
